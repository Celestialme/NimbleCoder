...
async function reset(){
 ...
 if($language!="upload"){
   ...
   $text_data = (await axios.get(`./${$language}_${$time}.txt`)).data.trim()
 }
characters = $text_data.split("\n").map(x=>x.trimEnd()).slice(0+total_line_count,show_number_of_lines+total_line_count).join("\n")
...
<div class="tooltip"><img src="./cursor.png" height=25 alt="cursor"> please click to resume</div>
...
{/if}
<h3 class:visible={!$interval || $finished}><img width=40 src="./github.png" alt="github icon"/>NimbleCoder 2022 ©</h3>
